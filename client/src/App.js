  ...
  const signIn=async(e)=>{
    ...
    await auth.signInWithPopup(provider)
      .then(result =>{
        ...
        authInfo.push(result)
        setAuthInfo([...authInfo])
        setCount(authInfo.length)

        if(authInfo){
          toast.success("Welcome " + authInfo[0].additionalUserInfo.profile.name,{position:"top-right"})
        }
      // setAuthInfo([])
      })
              ...
              <div className="Login__Form">
                <form>
                  <div className="Login__Profile">
                    <input className="file__Upload" type="file" />
                    <i className="fas fa-user  icon__block__user"></i>
                  </div>
